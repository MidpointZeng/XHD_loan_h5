  ...
  <div class="bankaccount-page">
    <div class="sc-fEOsli kVfqcf">
      <div class="sc-bjUoiL eynfbQ">
        <div class="sc-idiyUo iQiijW">お客様, +81 {{ userInfo.account }}</div>
        <div class="sc-dIouRR chsjmz">
          <img alt="" src="../../assets/img/avtar.png">
        </div>
      </div>
      <div class="sc-hHLeRK gKSmhE">
        <div class="sc-evZas dBtrgC"><span><img alt=""
              src="data:image/png;base64,"><span>お知らせ
              :</span></span>
          <div><span>{{ tips }}</span></div>
        </div>
        <h3 class="sc-dmRaPn fPIbLV">最大貸付可能額 (¥)</h3>
        ...
        <ul class="sc-fLlhyt dETFRX">
          <li>
            ...
            <div style="color: var(--system-light);">最も低い値段は〜から</div>
          </li>
          <li>
            <div> {{ Info.fast_time || 30 }}秒</div>
            <div style="color: rgb(243, 161, 109);">ウイックローン</div>
          </li>
        </ul>
        <div class="sc-bBrHrO hOhsct" @click="getUserInfo()">今すぐ借りる</div>
        <div class="sc-ivTmOn dMBgxr">基本認証（必要）</div>
        <ul class="sc-cxabCf fVHCyl">
          <li>
            <span v-if="userinfoinfo == null">未記入</span>
            <span v-else class="success">完了</span>
            <img alt=""
            ...
            <div>個人データ</div>
            <div v-if="userinfoinfo != null" @click="$router.push('/info1')">行く &gt;</div>
            <div v-else @click="$router.push('/submit')">行く &gt;</div>
          </li>
          <li>
            <span v-if="familyInfo == null">未記入</span>
            <span v-else class="success">完了</span>
            <img alt=""
            ...
            <div>求人情報</div>
            <div v-if="familyInfo != null" @click="$router.push('/info2')">行く &gt;</div>
            <div v-else @click="$router.push('/submit')">行く &gt;</div>

          </li>
          <li>
            <span v-if="bankInfo == null">未記入</span>
            <span v-else class="success">完了</span>
            <img alt=""
            ...
            <div>銀行情報</div>
            <div v-if="bankInfo != null" @click="$router.push('/info3')">行く &gt;</div>
            <div v-else @click="$router.push('/submit')">行く &gt;</div>
  ...
  methods: {
    ...
    getUserInfo2() {
      User.getUserInfo({ userId: this.userInfo.userId }).then(res => {
        ...
        this.userinfoinfo = res.data.userinfo
        window.localStorage.setItem('userInfo1', JSON.stringify(this.userinfoinfo))
        window.localStorage.setItem('userInfo2', JSON.stringify(this.familyInfo))
        window.localStorage.setItem('userInfo3', JSON.stringify(this.bankInfo))
...
.eynfbQ {
  display: flex;
  background: url(../../assets/img/ubg.4a183fcf11367d2b2333.png) center center / 100% no-repeat;
  height: 0rem;
  padding: 9rem 1.6rem;
...
.gKSmhE {
  ...
  .fVHCyl {
    ...
    li>div:nth-child(4) {
      ...
      margin: 1.6rem 0px 0px 1rem;
    }

    li:nth-child(2)>div:nth-child(4) {
      background: rgb(247, 225, 156);
    }
    ...
    li:nth-child(1) {
      background: url(../../assets/img/bg1.png) center center / 100% 100% no-repeat;
      border-color: rgb(191, 232, 241);
    }

    li:nth-child(2) {
      background: url(../../assets/img/bg2.png) center center / 100% 100% no-repeat;
      border-color: rgb(247, 225, 156);
    }

    li:nth-child(3) {
      background: url(../../assets/img/bg3.png) center center / 100% 100% no-repeat;
      border-color: rgb(216, 228, 253);
    }
  }
}
</style>
